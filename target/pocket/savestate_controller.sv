...
module savestate_controller #(
    ...
    // Core (clk_sys)
    input wire ss_allow,     // savestates usable right now (cart ready etc)
    input wire ss_busy,      // engine walk running
    input wire stage_lost,   // staging dropped a word of the blob (clk_mem)
  ...
  localparam SAVE_WAIT_BUSY = 3'd1;
  localparam SAVE_WALK = 3'd2;
  localparam SAVE_READBACK = 3'd3;
  localparam PRELOAD_STAGE = 3'd4;
  localparam LOAD_WAIT_STAGE = 3'd5;
  localparam LOAD_WAIT_BUSY = 3'd6;
  localparam LOAD_WALK = 3'd7;
  ...
  reg [6:0] reset_delay_count = 0;
  wire reset_settled = reset_delay_count[6];
  ...
  always @(posedge clk_sys_21_48) begin
    if (~ss_allow) begin
      reset_delay_count <= 0;
    end else if (~reset_settled) begin
  ...
  always @(posedge clk_sys_21_48) begin
    ...
    // Ack every request so core_bridge_cmd never hangs; outside the dispatching
    // states it is an error unless the same op is already running
    if (start_s & ~prev_start) begin
      start_ack <= 1;
      if (state != IDLE && ~start_busy) begin
        start_ok  <= 0;
        start_err <= 1;
      end
    end
    if (load_s & ~prev_load) begin
      load_ack <= 1;
      if (state != IDLE && state != PRELOAD_STAGE && ~load_busy) begin
        load_ok  <= 0;
        load_err <= 1;
      end
    end
    ...
    case (state)
      IDLE: begin
        ...
        if (start_s & ~prev_start) begin
          ...
          if (ss_allow && reset_settled) begin
            start_busy <= 1;
            ss_save <= 1;
      ...
      SAVE_WALK: begin
        if (~ss_busy && ~ss_allow) begin
          ...
          // the blob is incomplete, do not offer it
          start_busy <= 0;
          start_err <= 1;
          state <= IDLE;
      ...
      LOAD_WAIT_STAGE: begin
        if (last_write_s && stage_lost_s) begin
          // Staging dropped a word; loading the incomplete blob would wedge
          // the console
          load_busy <= 0;
          load_err <= 1;
          state <= IDLE;
        end else if (last_write_s && reset_settled) begin
